...
def change_background_color(selected_value: tuple, color: tuple, **kwargs):
    ...
    print(
        "Change widget color to", value_tuple[0]
    )  # selected_value format ('Color', surface, color)
...
if EXAMPLE == "BUTTON":
    ...
    menu.add.button(
        "Exec",
        func,
        "foo",  # Execute a function, it receives 'foo' as *arg
        align=pygame_menu.locals.ALIGN_LEFT,
    )
    btn = menu.add.button(
        about_menu.get_title(),
        about_menu,  # Open a sub-menu
        shadow=True,
        shadow_color=(0, 0, 100),
    )
    menu.add.button(
        "Exit",
        pygame_menu.events.EXIT,  # Link to exit action
    ...
    menu = make_menu(
        pygame_menu.themes.THEME_GREEN, "Drop Select Multiple", center=False
    )
    ...
    for i in range(9):
        frame_numbers.pack(
            menu.add.button(
                i, font_color=(5 * i, 11 * i, 13 * i), padding=(0, 5), font_size=30
            ),
            align=pygame_menu.locals.ALIGN_CENTER,
        )
    ...
    frame_content.pack(
        menu.add.toggle_switch(
            "Nice toggle", False, width=100, font_color=(150, 150, 150), padding=0
        ),
        align=pygame_menu.locals.ALIGN_CENTER,
    )
    ...
    menu = make_menu(
        pygame_menu.themes.THEME_DEFAULT, "Frame + Title", widget_font_size=18
    )
    menu.get_theme().widget_selection_effect.zero_margin()

    frame = menu.add.frame_v(
        400,
        800,
    ...
    menu.add.selector(
        "Change menu ",
        [("Menu 1", link1), ("Menu 2", link2), ("Menu 3", link3)],
        onreturn=open_link,
    )
    ...
    menu.add.progress_bar("My Progress", default=75.6)
    menu.add.progress_bar("Pygame-menu epicness?", default=99.9)

elif EXAMPLE == "RANGESLIDER":
    menu = make_menu(
        pygame_menu.themes.THEME_DEFAULT, "Range Slider", widget_font_size=18
    )
    ...
    table.add_row(
        [table_2, "", image], cell_vertical_position=pygame_menu.locals.POSITION_CENTER
    )
    table.update_cell_style(
        1, 2, padding=0
    )  # Disable padding for cell column 1, row 2 (table_2)
    ...
    menu.add.text_input(
        "Password: ", input_type=pygame_menu.locals.INPUT_INT, password=True
    )
    ...
    menu.add.url(
        "https://pygame-menu.readthedocs.io/en/master/", "pygame-menu documentation"
    )
